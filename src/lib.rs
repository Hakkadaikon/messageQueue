...
/// [`Queue::deliver`] が返す配送リース。`ack`/`nack` に渡して結果を確定する。
#[derive(Debug, Clone)]
pub struct Lease<T> {
    pub id: MsgId,
    pub payload: T,
...
/// ready ヒープのエントリ。順序は priority 降順 → seq 昇順(FIFO タイブレーク)。
/// Lean で証明した比較器 `better` と一致する。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct Slot {
    priority: i64,
    seq: u64,
...
struct Inner<T> {
    ...
    /// ack 済み msgId。二重 ack を冪等に弾く(SAFE-2)。
    acked: HashSet<MsgId>,
    next_id: u64,
    next_seq: u64,
    next_generation: u64,
...
impl<T: Clone> Queue<T> {
    ...
    /// メッセージは消えない(SAFE-3)。inflight は再 pending 化 or 上限超で DLQ。
...
/// 各状態の件数。`pending + inflight + acked + dead` は投入総数に一致する(SAFE-4)。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Counts {
    pub pending: usize,
    pub inflight: usize,
